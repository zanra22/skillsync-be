...
class YouTubeQualityRanker:
    ...
    def _calculate_quality_score(
        ...
        # TRANSCRIPT AVAILABILITY BOOST
        # If video has transcript, boost score by 20%
        # Rationale: Transcripts enable accessibility, AI analysis, and content verification
        if video.get('transcript_text'):
            total_score = min(100.0, total_score * 1.20)
            logger.debug(f"   ✓ Transcript available: +20% boost applied")
        else:
            logger.debug(f"   ✗ No transcript: Score kept as-is (fallback available via Groq)")
    ...
    def _calculate_transcript_score(
        ...
        Note: Videos WITHOUT transcripts get 0 points here, but the final score
        will use a 20% boost multiplier for videos WITH transcripts to prioritize them.
        This ensures transcript availability is rewarded without penalizing fallback options.
        """
        transcript = video.get('transcript_text', '')

        if not transcript:
            # No transcript = can't evaluate quality
            # Separate concern: boost multiplier handles transcript prioritization
            return 0.0  # No points for missing transcript (handled by multiplier)
